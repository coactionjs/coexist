...
Only `transport` is required on either factory; every other option defaults to a trusted, reliable channel. The remaining options group by concern — connect (`signal`), timeouts and recovery (`requestTimeout`, `readyTimeout`, `requestInitialSync`, `resync`), reachability (`expose`, `stateSections`, `sync`), cost control (`limits`, `includeErrorStack`, `serializeError`), and observers (`onConflict`, `onResync`, `onInvalidMessage`, `onDeliveryError`). See [the option surface](../../docs/worker-runtime.md#the-option-surface).
