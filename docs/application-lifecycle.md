...
- **A failed `createApp()` hands back its cleanup.** Creation is synchronous, so an async provider disposer cannot finish before the error is thrown. The error carries that disposal promise: `await getAppCreationCleanup(error)` resolves once everything creation had opened is released, and rejects with whatever release failed on. Ignoring it is safe — the rejection is already observed — but a caller that must know a connection actually closed should await it.
